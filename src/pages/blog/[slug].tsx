      ...
      "url": "${url}"
			...
			<Layout showGoBack>
				...
				<div className="flex flex-wrap mb-4">
					{post?.tags.map((tag) => (
						<span key={tag} className="p-1 bg-blue-100 dark:bg-blue-900 mr-2 rounded mt-2">
							#{tag.replace(/ /g, '')}
						</span>
					))}
				</div>
...
export const getStaticProps: GetStaticProps = async (context) => {
	const post = await new MarkdownPostRepository().getBySlug(context.params.slug.toString());
